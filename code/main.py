...
from display_game import afficher_jeu
...
cartes_main = []
cartes_main_small = []
main_Narouto = joueur1.hand

cartes_Narouto = joueur1.board

cartes_Sasouke = joueur2.board
...
def main():
    ...
    while current_player.PV > 0 and opponent.PV > 0:
        ...
        global nbr_play 
        etat["nbr_play"] = -1
        afficher_jeu(etat,joueur1.board,joueur2.board,opponent.hand,current_player.hand,cartes,cartes_small,cartes_main,cartes_main_small,turn)
        while etat["nbr_play"] != 0:


            etat["nbr_play"] = int(input(f"\n{current_player.name}: Choississez la carte que vous voulez jouer dans la main (avec le nombre de l'index),si vous voulez attacker tapez -1 ou sinon tapez 0 pour passez votre tour? "))
            if etat["nbr_play"] != 0:
                if etat["nbr_play"] != -1:
                    ...
                    current_player.play_card(current_player.hand[etat["nbr_play"]-1], row, col,current_player)
                    print(f"\n{opponent.name}: ")
                    opponent.show_board()
                    print(f"\n{current_player.name}: ")
                    current_player.show_board()
