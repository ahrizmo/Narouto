...
class Player:
    def __init__(self, name: str, pv: int, chakra: int):
        self.name = name
        self.PV = pv
        self.chakra = chakra
        self.deck = deque()  # Pile de cartes
        self.hand = [None for _ in range(5)] # Liste de cartes en main
    ...
    def draw(self):
        ...
        if self.deck:
            i = 0
            while self.hand[i] is not None:
                i += 1
            
            drawed_card = self.deck.pop()
            self.hand[i] = drawed_card
